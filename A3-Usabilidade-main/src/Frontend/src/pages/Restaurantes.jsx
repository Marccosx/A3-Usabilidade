...
const Restaurantes = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (editando) {
                response = await axios.put(`http://localhost:3000/restaurantes/${editando}`, novoRestaurante);
            } else {
                response = await axios.post('http://localhost:3000/restaurantes', novoRestaurante);
            }
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Tem certeza que deseja excluir este restaurante?')) {
            try {
                const response = await axios.delete(`http://localhost:3000/restaurantes/${id}`);
                if (response.data.success) {
                    carregarRestaurantes();
                    setErro('');
                } else {
